...
public class EventStoreBatchContainer : IBatchContainer
{
    ...
    public EventStoreBatchContainer(Serializer serializer)
    {
        ...
        : this(serializer)
    {
        ...
        Token = new EventStoreSequenceToken(eventStoreMessage.Position, eventStoreMessage.SequenceNumber, 0);
    }
    ...
    [JsonProperty]
    [Id(1)]
    private EventStoreSequenceToken Token { get; set; } = new();
    ...
    {
        return Payload.Events.Cast<T>().Select((evt, index) => Tuple.Create<T, StreamSequenceToken>(evt, new EventStoreSequenceToken(Token.Position, Token.SequenceNumber, index)));
    }
